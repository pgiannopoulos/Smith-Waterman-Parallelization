/*****************************************************
* Στεφάνου Ελευθερία 			ΑΜ:	5940 			 *
* Λύρας Αθανάσιος 	 			ΑΜ:	5338			 *
* Τσίχλας Ιωάννης	 			ΑΜ:	5671			 * 
* Γιαννόπουλος Παναγιώτης		ΑΜ:	5496			 *
*****************************************************/
...
int main(int argc, char** argv) {
	...
	int i,j,tempor2,tempor;
    gettimeofday(&StartTime, NULL);
	
    for( int i=N_b-2; i>-N_a; i-- ) 
	{
		#pragma omp parallel 
		{
		int tempor=N_a+i;
		if (i<0){tempor2=0;}else{tempor2=i+1;}
		#pragma omp for schedule(static) 
        for( j=tempor2; j<N_b ; j++ ) 
		{
			if ( j<tempor){
			int ii = j-i; // neo i
			int jj = N_b-j; // neo j
            temp[0] = H[ii-1][jj-1]+similarity_score(seq_a[ii-1],seq_b[jj-1]);
            temp[1] = H[ii-1][jj]-delta;
            temp[2] = H[ii][jj-1]-delta;
            temp[3] = 0.;
			#pragma omp critical
            H[ii][jj] = find_array_max(temp, 4);
            if(ind==0) {                      		 // score in (i,j) stems from a match/mismatch
                I_i[ii][jj] = ii-1;
                I_j[ii][jj] = jj-1;
				}
            else if(ind==1) {                             // score in (i,j) stems from a deletion in sequence A
                I_i[ii][jj] = ii-1;
                I_j[ii][jj] = jj;
				}	
            else if(ind==2) {                            // score in (i,j) stems from a deletion in sequence B
                I_i[ii][jj] = ii;
                I_j[ii][jj] = jj-1;
				}
            else if(ind==3) {                            // (i,j) is the beginning of a subsequence
                I_i[ii][jj] = ii;
                I_j[ii][jj] = jj;
				}
            }
